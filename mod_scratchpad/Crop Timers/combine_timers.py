...
def find_crop_sprite_bounds(img: Image.Image, outline: tuple):
    """
    Return (left, right) column indices of the bounding box of outline pixels,
    used to determine the sprite's horizontal centre.
    """
    pixels = img.load()
        ...
        for x in range(img.width):
            if pixels[x, y] == outline:
                ...
                if x > max_x:
                    max_x = x
    if max_x == -1:
        raise ValueError(f"No outline pixel {outline} found when computing sprite bounds.")
    return min_x, max_x
...
def compose(crop_img: Image.Image, timer_img: Image.Image) -> Image.Image:
    ...
    crop_left, crop_right = find_crop_sprite_bounds(crop_img, outline)

    # Horizontal centre of the crop sprite (pixel index, 0-based)
    crop_sprite_width = crop_right - crop_left + 1
    # Integer centre; for odd widths this is left-biased (floor division).
    # "justify left 1 px if necessary" == floor, which is the default for //.
    crop_centre_x = crop_left + crop_sprite_width // 2
